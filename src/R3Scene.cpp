...
Read(const char *filename, R3Node *node)
{
    ...
    while (fscanf(fp, "%s", cmd) == 1) {
        if (cmd[0] == '#') {
            ...
            if (lifetime > eps) particle->lifetimeactive = true;
            else particle->lifetimeactive = false;
            ...
            player->shape = shape;
	    player->shape->mesh->Scale(0.2, 0.2, 0.2);
            player->pos = p;
            player->nose = n;
            player->wing = w;
            player->velocity = vel;
	    ...
	    player->shape->mesh->Rotate(M_PI, R3Line(player->shape->mesh->Center(), R3Vector(0, 1, 0)));
            // Add particle to scene
            ...
            if (lifetime > eps) source->lifetimeactive = true;
            else source->lifetimeactive = false;
            ...
            enemy->shape = shape;
	    //	    enemy->shape->mesh->Scale(2.0, 2.0, 2.0);
            enemy->remainder = 0;
